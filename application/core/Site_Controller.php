...
class Site_Controller extends CI_Controller
{
    ...
    public function UploadMultiImages($productsID='',$table_file='files',$relationship='real_news_images'){
		...
		if($this->upload->do_multi_upload("Images_upload")){
        	...
        	$temp = $this->db->query("select count(*) as sl from real_news_images where NewsID=$productsID")->row();
			$temp = $temp ? 5-$temp->sl : 5;
			$max=count($_FILES['Images_upload']["name"])>$temp ? $temp : count($_FILES['Images_upload']["name"]);
			for($i=0; $i<$max; $i++){
		        $data = array(
    ...
    // upload multiImages
    public function UploadMultiImages_sanpham($productsID='',$table_file='files',$relationship='gianhang_sanpham_images'){
        $this->make_folder($this->upload_to,'assets/');
        //Configure upload.
        $this->load->library("upload");
        $this->upload->initialize(array(
        ...
        if($this->upload->do_multi_upload("Images_upload")){
            $data_file = $this->upload->get_multi_upload_data();
            $temp = $this->db->query("select count(*) as sl from $relationship where SanphamID=$productsID")->row();
            $temp = $temp ? 5-$temp->sl : 5;
            $max=count($_FILES['Images_upload']["name"])>$temp ? $temp : count($_FILES['Images_upload']["name"]);
            for($i=0; $i<$max; $i++){
                $data = array(
                ...
                $IDmax = $this->db->query("select max(ID) as maxID from $table_file")->row();
                if($productsID!='' && is_numeric($productsID)){
                    $this->db->query("insert into $relationship(ImageID,SanphamID) value($IDmax->maxID,$productsID)");
                }
                $url_array[] = "assets/".$this->upload_to.'/'.$data_file[$i]['file_name'];
            }
            $this->created_thumb_image($url_array,200,200,$table_file);
    ...
    public function delete_file_by_id_file($id,$table_file='file'){
        if(is_numeric($id)){
            ...
            if($path){
                ...
                if(file_exists($base_path.$path['Thumb'])){
                    unlink("./".$path['Thumb']);
                }
